...
const COPY = {
  credit: { title: "Add Funds", lede: "Find a customer by email and credit their balance directly.", prompt: "Search for a customer by email to add funds to their account." },
  debit: { title: "Debit Funds", lede: "Find a customer by email and debit their balance directly.", prompt: "Search for a customer by email to debit funds from their account." },
} as const;

/** Shared customer-search-then-act shell behind the Administration
 * sidebar's "Add Funds" and "Debit Funds" tools — separate sidebar
 * entries, each locking AddFundsPanel to its own direction, rather than
 * one combined control. Finds the customer by email, then hands off to
 * AddFundsPanel. */
export function FundsActionPage({ direction }: { direction: "credit" | "debit" }) {
      ...
      {selected ? (
        <Panel>
          <PanelBody>
            ...
            <AddFundsPanel userId={selected.id} token={session.token} fixedDirection={direction} />
          </PanelBody>
        </Panel>
      ) : (
        <Panel>
          <PanelBody>
            ...
            {!search ? (
              <p className="text-center py-8 text-ink-2 text-[12.5px]">{copy.prompt}</p>
            ) : loading ? (
